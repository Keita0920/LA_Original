...
class HomeViewController: UIViewController, CLLocationManagerDelegate , MKMapViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate{
    ...
    func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
        if (view.annotation is MKUserLocation) {
            ...
            guard let myAnnotation = view.annotation as? AnnotationPin  else {
                return
            }
            self.selectedImage=myAnnotation.object
        }
...
class AnnotationPin:  NSObject, MKAnnotation {
    ...
    let object:Image!
    let selectedTag:Int?
    init(title:String?=nil, subtitle: String?=nil, image: UIImage, coordinate: CLLocationCoordinate2D,object:Image,selectedTag:Int?=nil) {
        self.title = title
